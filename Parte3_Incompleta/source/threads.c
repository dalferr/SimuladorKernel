...
//Ahora el proces_generator es el LOADER
void* process_gen(void* arg) {
  while(1){
    sem_wait(&sem2); //se queda esperando a timer
      //PCB proc;
    ...
    if (numFich < 10){
      char nombreFich[23];
      snprintf(nombreFich, sizeof(nombreFich), "prometheus/prog00%d.elf", numFich);
      numFich++;
      cargarFichero(nombreFich, proc);
    }
    else {
      char nombreFich[23];
      snprintf(nombreFich, sizeof(nombreFich), "prometheus/prog0%d.elf", numFich);
      numFich++;
      cargarFichero(nombreFich, proc);
    }
